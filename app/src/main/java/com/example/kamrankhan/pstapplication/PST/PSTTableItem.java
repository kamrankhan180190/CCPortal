...
class PSTTableItem {
	...
	public long getLongValue() {
		if ( this.data.length > 0 ) {
			return com.example.kamrankhan.pstapplication.PST.PSTObject.convertLittleEndianBytesToLong(data);
		}
		return -1;
	}
...
/**/
	...
	public String toString() {
		String ret = com.example.kamrankhan.pstapplication.PST.PSTFile.getPropertyDescription(entryType, entryValueType);
		...
		if ( entryValueType == 0x0005 ||
			...
			if ( data.length == 8 ) {
				long l = com.example.kamrankhan.pstapplication.PST.PSTObject.convertLittleEndianBytesToLong(data, 0, 8);
		...
		if ( entryValueType == 0x0040 ) {
			// It's a date...
			int high = (int) com.example.kamrankhan.pstapplication.PST.PSTObject.convertLittleEndianBytesToLong(data, 4, 8);
			int low = (int) com.example.kamrankhan.pstapplication.PST.PSTObject.convertLittleEndianBytesToLong(data, 0, 4);
