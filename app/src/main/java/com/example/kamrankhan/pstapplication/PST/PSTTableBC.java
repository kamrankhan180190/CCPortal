...
class PSTTableBC extends com.example.kamrankhan.pstapplication.PST.PSTTable {
	...
	PSTTableBC(PSTNodeInputStream in)
		throws com.example.kamrankhan.pstapplication.PST.PSTException, java.io.IOException
	{
		super(in, new HashMap<Integer, com.example.kamrankhan.pstapplication.PST.PSTDescriptorItem>());
    ...
    descBuffer.append("Number of entries: " + numberOfKeys + "\n");
		...
		for (int x = 0; x < numberOfKeys; x++) {
			...
			item.itemIndex = x;
			item.entryType =(int) com.example.kamrankhan.pstapplication.PST.PSTObject.convertLittleEndianBytesToLong(keyTableInfo, offset+0, offset+2);
			//item.entryType =(int)in.seekAndReadLong(offset, 2);
			item.entryValueType = (int) com.example.kamrankhan.pstapplication.PST.PSTObject.convertLittleEndianBytesToLong(keyTableInfo, offset+2, offset+4);
